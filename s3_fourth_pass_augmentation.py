# s3_fourth_pass_augmentation.py (智能四阶段终极补录版)
import json
import pandas as pd
import tqdm
import numpy as np
...
def main():
    ...
    SECOND_PASS_AUG_PATH = 'aug_sentences_second_pass.csv'
    THIRD_PASS_AUG_PATH = 'aug_sentences_third_pass.csv' # 第三轮的成果

    # 输出文件
    FOURTH_PASS_OUTPUT_PATH = 'aug_sentences_fourth_pass.csv' # 本次“第四轮补录”的输出
    FINAL_REPORT_PATH = 'augmentation_summary_report_final_v2.txt' # 更新最终报告文件名
    GOLDEN_SAMPLES_OUTPUT_PATH = 'stubborn_categories_golden_samples_pass4.csv' # 第四轮的黄金样本
    
    TARGET_SAMPLE_COUNT_THIS_PASS = 6 # 本轮的目标是至少达到5个
    
    # 实时自适应动态阈值配置 (可以考虑在本轮进一步放宽上限)
    STRICT_ENTROPY_THRESHOLD = 2.3
    MAX_LENIENT_THRESHOLD = 3.8 # 进一步放宽上限，接近理论最大值3.9
    SUCCESS_RATE_TRIGGER = 0.2 # 触发条件也可以进一步放宽，例如5%
    ...
    try:
        ...
        third_pass_df = pd.read_csv(THIRD_PASS_AUG_PATH) # 加载第三轮结果
        print("所有数据文件加载成功。\n")
    except FileNotFoundError as e:
        print(f"\n[错误!] 文件未找到: {e}！请确保前三轮的文件都在。程序终止。")
        return
    ...
    # --- 3. 诊断当前状态，识别需要“第四轮补录”的类别 (<5) ---
    print("--- 步骤 3/5: 诊断当前数据集状态，识别需要进行“第四轮补录”的类别 ---")
    ...
    third_pass_counts = third_pass_df['tech_id'].value_counts() # 读取第三轮计数
    # 计算当前总数 = 原始 + Pass1 + Pass2 + Pass3
    current_total_counts = initial_counts.add(first_pass_counts, fill_value=0).add(second_pass_counts, fill_value=0).add(third_pass_counts, fill_value=0)
    ...
    print(f"诊断完成，共识别出 {len(fourth_pass_targets)} 个样本数仍 < {TARGET_SAMPLE_COUNT_THIS_PASS} 的类别，将对它们进行第四轮补录。\n")

    # --- 4. 核心生成与过滤循环 (只针对第四轮目标) ---
    print("--- 步骤 4/5: 开始对目标类别进行第四轮生成与过滤 ---")
    fourth_pass_sentences = []
    golden_pass_sentences_pass4 = []

    for tech_id in tqdm.tqdm(fourth_pass_targets, desc="第四轮补录"):
...
"""
        ...
        for retry_count in range(MAX_LLM_RETRIES):
            ...
            # 实时自适应动态阈值逻辑 (与上一版相同，但阈值上限和触发条件可能已调整)
            if retry_count > 0:
                ...
                if success_rate < SUCCESS_RATE_TRIGGER and rejected_entropies:
                    ...
                    # 建议值可以更激进一点，比如直接用中位数？或者用75分位数？这里我们保持之前的策略
            ...
            try:
                ...
                time.sleep(2)
                continue
        fourth_pass_sentences.extend(generated_for_this_tech)
        golden_pass_sentences_pass4.extend(golden_for_this_tech)

    # --- 5. 保存并生成最终报告 ---
    print(f"\n--- 步骤 5/5: 保存结果并生成最终分析报告 ---")
    pd.DataFrame(fourth_pass_sentences).to_csv(FOURTH_PASS_OUTPUT_PATH, index=False)
    print(f"第四轮（终极补录）过程完成，所有被接纳的 {len(fourth_pass_sentences)} 个样本已保存到 '{FOURTH_PASS_OUTPUT_PATH}'。")
    
    pd.DataFrame(golden_pass_sentences_pass4).to_csv(GOLDEN_SAMPLES_OUTPUT_PATH, index=False)
    print(f"在第四轮补录过程中，共发现 {len(golden_pass_sentences_pass4)} 个'黄金样本'（熵<1.0），已单独保存到 '{GOLDEN_SAMPLES_OUTPUT_PATH}'。")

    print(f"正在生成最终的统计报告 '{FINAL_REPORT_PATH}'...")
    fourth_pass_counts = pd.Series(fourth_pass_sentences).apply(lambda x: x['tech_id']).value_counts()
    # 最终总数 = 原始 + Pass1 + Pass2 + Pass3 + Pass4
    final_total_counts = current_total_counts.add(fourth_pass_counts, fill_value=0)
    ...
    for tech_id in sorted(graph_map.keys()):
        ...
        pass3_gen = int(third_pass_counts.get(tech_id, 0))
        pass4_gen = int(fourth_pass_counts.get(tech_id, 0)) # 新增第四轮计数
        final_total = int(final_total_counts.get(tech_id, 0))
        
        status = f"已达标 (>= {TARGET_SAMPLE_COUNT_THIS_PASS})" if final_total >= TARGET_SAMPLE_COUNT_THIS_PASS else f"仍未达标 (< {TARGET_SAMPLE_COUNT_THIS_PASS})"
    ...
    with open(FINAL_REPORT_PATH, 'w', encoding='utf-8') as f:
        f.write("="*100 + "\n")
        f.write("                         数据集增强最终状态分析报告 (四轮后)\n")
        f.write("="*100 + "\n\n")
        
        summary = (
            f"前两轮目标样本数/类别: 50\n"
            f"第三轮目标样本数/类别: 10\n"
            f"第四轮目标样本数/类别: {TARGET_SAMPLE_COUNT_THIS_PASS}\n"
            f"总技术类别数: {len(report_df)}\n"
            f"最终达标的类别数 (>=5): {len(report_df[report_df['Status'] == f'已达标 (>= {TARGET_SAMPLE_COUNT_THIS_PASS})'])}\n"
            f"最终仍未达标的类别数 (<5): {len(report_df[report_df['Status'] == f'仍未达标 (< {TARGET_SAMPLE_COUNT_THIS_PASS})'])}\n\n"
        )
        f.write("--- 总体概览 ---\n")
        f.write(summary)
